...
def generate_json_schema(tools):
    ...
    for tool in tools:
        ...
        schema['items']['allOf'].append({
            ...
            "then": {
                "properties": {
                    # Directly set the schema instead of referencing it
                    "arguments": function_schema 
                }
            }
        })
...
class Tool(BaseModel):
    ...
    def sync_arguments(cls, values):
        ...
        work with in Python. So when we load from an OAI request, arguments is set,
        and we set arguments_dict to json.loads of arguments, and visa versa.
        """
        ...
        if arguments is not None:
            if isinstance(arguments, str):
                ...
                try:
                    ...
                    raise ValueError(f"Invalid JSON string in `arguments` field: {arguments}")
            elif isinstance(arguments, dict):
                values["arguments_dict"] = arguments
                values["arguments"] = json.dumps(arguments)
            else:
                raise Exception(
                    f"Tool.arguments encountered unexpected type, expected str or dict, but got: {type(arguments)}"
                )
